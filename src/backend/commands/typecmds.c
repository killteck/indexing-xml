...
DefineRange(CreateRangeStmt *stmt)
{
	...
	/* Check we have creation rights in target namespace */
	aclresult = pg_namespace_aclcheck(rangeNamespace, GetUserId(), ACL_CREATE);
	if (aclresult != ACLCHECK_OK)
		aclcheck_error(aclresult, ACL_KIND_NAMESPACE,
					   get_namespace_name(rangeNamespace));
	 ...
	 */
	old_type_oid = GetSysCacheOid2(TYPENAMENSP,
								   CStringGetDatum(rangeName),
								   ObjectIdGetDatum(rangeNamespace));
	if (OidIsValid(old_type_oid))
	{
		if (!moveArrayTypeName(old_type_oid, rangeName, rangeNamespace))
			ereport(ERROR,
					(errcode(ERRCODE_DUPLICATE_OBJECT),
					 errmsg("type \"%s\" already exists", rangeName)));
	}
	 ...
	 * Create the array type that goes with it.
	 */
	rangeArrayName = makeArrayTypeName(rangeName, rangeNamespace);
